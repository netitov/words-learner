...
function Quiz(props) {
  ...
  function setNextQuestion(i) {
    ...
    if (passedQuests.some((el) => el.translation === i.translation)) {
      ...
      const currPosition = questions.findIndex((p) => p._id === activeQuestion._id);
      const quest =
        currPosition + 1 > questions.length - 1 ? { word: '' } : questions[currPosition + 1];
      setActiveQuestion(quest);

      // progress line
      const nextQuestPos = questions.findIndex((el) => el.translation === quest.translation);
      setProgress({ percent: `${completedPart}%`, quest: nextQuestPos + 1 });

      // quiz result
      if (completedPart === 100) {
        const correctAnswer = passedQuests.filter((q) => q.correct).length;
  ...
  function handleAnswerBtn(text) {
    ...
    if (answeredQuestion.answer === text) {
  ...
  /* function createQuizQuestions() {
    ...
    mainWords.forEach((i) => {
      ...
      const filteredWords = allWords.filter((el) => {
        return (
          el.word !== obj.correctAnswer && el.syn !== obj.correctAnswer
        );
      });
      ...
      <div className='quiz__container'>
        ...
        <div
          className={`quiz__progress${
            progress.percent === '100%' ? ' quiz__progress_inactive' : ''
          }`}
        >
          <div className='quiz__progress-done' style={{ width: progress.percent }} />
          <span>
            {progress.quest} / {questions.length}
          </span>
        </div>
        ...
        <div className={`quiz__result${progress.percent === '100%' ? ' quiz__result_active' : ''}`}>
          <div className='quiz__chart'>
            ...
            <span className='quiz__chart-result'>
              {Math.round((result.correctAnsw / result.quests) * 100)}%
            </span>
          </div>

          <div className='quiz__comment-box'>
            <p className='quiz__progress-comment'>
              Your result:{' '}
              <span>
                {result.correctAnsw} / {result.quests}
              </span>
            </p>
          ...
          <div className='quiz__btn-box'>
            <button className='quiz__btn quiz__btn-result' type='button' onClick={startOver}>
              Start over
            </button>
            {/* if test is taken from user account shuffle saved words, otherwise update random words */}
            <button className='quiz__btn quiz__btn-result' type='button' onClick={updateWords}>
              {props.account ? 'Continue' : 'Update words'}
            </button>
            <button className='quiz__btn quiz__btn-result' type='button' onClick={handleClose}>
              Close
            </button>
          </div>
        ...
        {questions.map((i) => (
          <div
            className={`quiz__quest-box${
              i._id === activeQuestion._id ? ' quiz__quest-box_active' : ''
            }`}
            key={i._id}
          >
            {/* question box */}
            ...
            <div className='quiz__options-box'>
              {i.options.map((word) => (
                ...
                >
                  <span>{word}</span>
                  {handleAnswerBtn(word).includes(' quiz__option_correct') ? (
                    <IoCheckmarkCircleOutline />
                  ) : handleAnswerBtn(word).includes(' quiz__option_incorrect') ? (
            ...
            <button
              ...
              className={`quiz__btn quiz__btn-next${
                passedQuests.some((el) => el.translation === i.translation)
        ...
        {/* loading overlay */}
        <div className={`quiz__overlay${isLoading ? ' quiz__overlay_active' : ''}`}>
