...
function Translate(props) {
  ...
  function toggleLangList(e) {
    ...
    if (type === langListActive.type) {
      ...
      setLangListActive({ type, value: true });
  ...
  function findSavedWords(translatedWords, savedWords) {
    ...
    translatedWords.forEach((translatedWord) => {
      translatedWord.tr?.forEach((word) => {
        ...
        } else if (savedWords.find((sw) => sw.word === word.text)) {
          foundTranslations.push(word.text);
        ...
        word?.syn?.forEach((synonym) => {
          ...
          } else if (savedWords.find((sw) => sw.word === synonym.text)) {
            foundTranslations.push(synonym.text);
          }
        });
      });
      ...
      translatedWord.tr?.forEach((word) => {
        word?.mean?.forEach((synonym) => {
          if (isDifferentLang) {
            ...
            savedWords.find((sw) => sw.word === translation && sw.translation === synonym.text)
          ) {
            foundTranslations.push(synonym.text);
          }
        });
  ...
  function updateDictionaryState(wordsArr, translation) {
    ...
    const savedWordsSyn =
      wordsArr.length > 0 ? findSavedSyns(wordsArr, userWords, translation) : [];
  ...
  async function compareFreq(data, translation) {
    ...
    const foundFreqs = !response.some((i) => i.word === frequency.word)
      ? [frequency, ...response]
      : response;
  ...
  return (
    <div
      ...
      id='translator'
      style={{ minHeight: `${translatorHeight}px` }}
      ref={compRef}
    >
      ...
      <div className={`translator${animation ? ' translator_active' : ''}`} ref={pathRef}>
        ...
        <div className='translator__box translator__box_input'>
          ...
          <div className='translator__container translator__container_input'>
            ...
            <div className='translator__secondary-container'>
              <div
                className={`translator__freq-box${
                  frequency.text.length > 0 && currentInputLang.lang === 'English'
              ...
              >
                ...
                >
                  <button className='translator__tlt-btn' type='button'>
                    <AiOutlineQuestionCircle size='15' /* color='#757575' */ />
                  </button>
                </Tooltip>
              </div>
              <span>
                {chars.length} / {charsLimit}
              </span>
            </div>
        ...
        <Tooltip title='swap languages' componentsProps={{ tooltip: { sx: tooltipOption } }}>
          <button className='translator__swap-btn' type='button' onClick={swapLangs}>
            <svg id='_Слой_1' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 288.33 145.4'>
              ...
              <polygon
                className='cls-1'
                points='288.33 35.31 257.75 52.96 227.17 70.62 227.17 35.31 227.17 0 257.75 17.65 288.33 35.31'
              />
              <rect className='cls-1' x='24.48' y='108.06' width='263.84' height='4.08' />
              <polygon
                className='cls-1'
                points='0 110.09 30.58 127.75 61.16 145.4 61.16 110.09 61.16 74.78 30.58 92.44 0 110.09'
              />
            </svg>
          </button>
        </Tooltip>
        ...
        <div className='translator__box translator__box_output'>
          ...
          <div className='translator__container translator__container_output'>
            ...
            <div className='translator__secondary-container'>
              <div
                className={`translator__freq-box${
                  frequency.text.length > 0 && currentOutputLang.lang === 'English'
                    ? ' translator__freq-box_active'
                    : ''
                }`}
              >
        ...
        <Dictionary
          ...
          handleLearnList={(word, wordTransl, isSaved) =>
            handleWordList(word, wordTransl, isSaved, true)
          } // true: call from dictionary
