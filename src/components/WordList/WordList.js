...
function WordList() {
  ...
  function updateQuiz() {
    ...
    const wordsToLearnSlice = (wordsToLearn.length > 0 ? wordsToLearn : initWords).slice(
      0,
      quizQuests,
    );
  ...
  function getProgress(arr) {
    ...
    return correctAnswers === 0
      ? 0
      : Math.min(Math.round((correctAnswers / minPassedTests) * 100), 100);
  }
  ...
  function getWordsData(wordsArray, currentCollectionId) {
    ...
    const wordsToLearn =
      notLearnedWords.length > 0 ? notLearnedWords : wordsSlice.slice(0, quizQuests);
        ...
        <table className='wordlist__table wordlist-table' ref={pathRef}>
          <thead>
            <tr>
              ...
              <th>word</th>
              <th className='wordlist-table__th wordlist-table__th_btn' onClick={toggleLangList}>
                translation
          ...
          <tbody>
            ...
            {(isFilterActive ? filteredWords : words).map((i) => (
              <tr key={i.word} className='wordlist-table__row'>
                <td>
                  <Bookmark
                ...
                <td>
                  <div className='wordlist-table__progress'>
                    <div className='wordlist-table__progress-chart'>
                      <div
                        className='wordlist-table__progress-done'
                        style={{ width: `${getProgress(i.results)}%` }}
                      />
                    </div>
